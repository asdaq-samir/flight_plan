...
export default function SettingsButton() {
  ...
  return (
    <Button
      ...
      aria-label="Settings"
    >
      {/* state.from -- not a query param on /settings itself -- is what
          lets Settings' own back button read "where you came from"
          (map, briefing, or Label) and both label and return there
          exactly, including the briefing's own ?view=briefing. Plain
          location state, not lifted into every caller's own props: this
          is the one place in the app that already knows the current
          route without being told. */}
