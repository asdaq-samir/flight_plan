...
export default function WaypointList({
  ...
  // the "n of total" the map popup shows for the same point.
  let n = 0;

  return (
    <Card title="Waypoints" className="min-h-0 flex-1 overflow-y-auto">
          ...
          return (
            ...
            >
              <div className="flex items-baseline justify-between gap-2">
                <span>{p.ident}</span>
                <Badge color="#142430">{p.category === "departure" ? "DEP" : "DEST"}</Badge>
              </div>
          ...
          >
            <div className="flex items-baseline justify-between gap-2">
              ...
              <span className="flex items-center gap-1">
                ...
                {roleOf(p) === "visual" && <Badge color="#7b3fa0">visual</Badge>}
                <Badge color={COLORS[rating]}>{rating}</Badge>
              </span>
            </div>
