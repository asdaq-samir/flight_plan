...
| `charts.py` | The same FAA GeoTIFFs rendered as an XYZ tile pyramid -- sectionals, TACs, IFR enroute and IFR area charts -- clipped to each sheet's face (found by `chart_faces.py`) so adjacent sheets butt together and no street map shows through the gaps, one drawing order for the pyramid and for tiles rendered on demand (they used to disagree inside sheet overlaps), and the sectional's masked lines -- the paper band round each TAC and inset -- taken back out when a sheet is prepared. `planning-service`'s `/api/chart-tile/*` and the batch `prepare`/`pyramid`/`refresh`/`unmask` commands (see that service's own README) are this module's CLI. |
| `chart_faces.py` | Finding a chart's face on its raster -- the neatline, or where the chart runs out -- and cleaning the raster: the lines the FAA's own masking left, found and given their tint back. Split out of `charts.py` because it changes on its own. |
...
| `narrative.py` | What both narrative agents write from: the request they accept (`NarrativeRequest`) and the one briefing prompt, so the framework comparison is on one task. Its pydantic is on vfr's list like any other dependency; the agents get theirs with their frameworks. |
